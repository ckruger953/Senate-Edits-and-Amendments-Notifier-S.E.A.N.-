            ...
            for row in docket_rows:
                ...
                if bn not in bill_map:
                    bill_map[bn] = {
                        ...
                        'floor_status': 'Pending',  # Pending, Concurred, Nonconcurred, CofC, Enrolled
                    }
                ...
                # Lowercase desc for keyword matching (used by both floor status and amendment detection)
                desc_lower = desc.lower()

                # Detect House floor action status (concur/nonconcur/CofC/enrolled)
                if body == 'H':
                    if 'concur' in desc_lower and 'nonconcur' not in desc_lower and 'reconsider' not in desc_lower:
                        bill_map[bn]['floor_status'] = 'Concurred'
                    elif 'nonconcur' in desc_lower and 'request committee of conference' in desc_lower:
                        bill_map[bn]['floor_status'] = 'CofC Requested'
                    elif 'nonconcur' in desc_lower:
                        bill_map[bn]['floor_status'] = 'Nonconcurred'
                    elif 'conference committee report' in desc_lower or 'c of c' in desc_lower or 'cofc' in desc_lower:
                        bill_map[bn]['floor_status'] = 'CofC'
                    elif 'enrolled' in desc_lower:
                        bill_map[bn]['floor_status'] = 'Enrolled'
