    ...
    try:
        ...
        if qualifying_bills:
            ...
            for row in docket_rows:
                ...
                # Capture House actions (for client-side floor status detection)
                if body == 'H':
                    bill_map[bn]['house_actions'].append({
                        'date': date_str,
                        'description': desc,
                    })
            ...
            # Step 5: Fetch amendment texts for all amendments
            import urllib.request
            import html as html_module
            for bn in bill_map:
                bill_map[bn]['amendment_texts'] = {}
                for num in bill_map[bn]['amendment_numbers']:
                    try:
                        url = f"https://gc.nh.gov/bill_status/pdf.aspx?q=amendment&id={num}"
                        req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
                        with urllib.request.urlopen(req, timeout=5) as resp:
                            raw = resp.read().decode('utf-8', errors='replace')
                            # Extract text content — the response is HTML, extract the body text
                            # Simple extraction: strip tags
                            import re as re2
                            text = re2.sub(r'<[^>]+>', '', raw)
                            text = html_module.unescape(text).strip()
                            if text and len(text) > 50:
                                bill_map[bn]['amendment_texts'][num] = text
                    except Exception:
                        pass  # Skip amendments we can't fetch
